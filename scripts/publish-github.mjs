#!/usr/bin/env node
/**
 * Publishes all workspace packages to GitHub Packages under the @vankyle-hub scope.
 *
 * The canonical package names in this repository are @vankyle/* (npm-first).
 * GitHub Packages requires packages to use @vankyle-hub (matching the GitHub
 * owner), so this script temporarily rewrites each package.json — changing the
 * scope and resolving workspace:* dependencies to concrete versions — then runs
 * `pnpm publish`.
 */
...
const SCOPE_FROM = "@vankyle/";
const SCOPE_TO = "@vankyle-hub/";
...
for (const dir of packageDirs) {
  ...
  try {
    ...
    // Rewrite @vankyle/ → @vankyle-hub/ in compiled dist files so that the
    // published package resolves its imports against the GitHub Packages scope.
